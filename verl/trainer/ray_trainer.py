...
class RayPPOTrainer:
    ...
    def _maybe_log_val_generations(
        ...
        # sample_dicts = [
        #     {"input": inp, "output": out, "label": lab, "score": sc}
        #     for inp, out, lab, sc in samples
        # ]
        # save_dir = '/opt/tiger/UniVerifier/'
        # os.makedirs(save_dir, exist_ok=True)
        # json_path = os.path.join(save_dir, f"val_generations_step{self.global_step}.json")
        # with open(json_path, "w", encoding="utf-8") as f:
        #     json.dump(sample_dicts, f, ensure_ascii=False, indent=4)
    ...
    def _validate(self) -> dict[str, Any]:
        ...
        # Modified: Use nested defaultdict to group metrics by task
        reward_metrics_lst = defaultdict(lambda: defaultdict(list))
        length_metrics_lst = defaultdict(lambda: defaultdict(list))
        ...
        for batch_dict in tqdm(self.val_dataloader):
            ...
            # Modified: Group metrics by task if task information is available
            if "task" in test_batch.non_tensor_batch:
                tasks = test_batch.non_tensor_batch["task"].tolist()
                for task in set(tasks):
                    task_indices = [i for i, t in enumerate(tasks) if t == task]
                    for key, value in reward_metrics.items():
                        task_values = [value[i] for i in task_indices]
                        reward_metrics_lst[task][key].extend(task_values)
                    
                    # Calculate length metrics for this task
                    task_batch_dict = {k: v[task_indices] if hasattr(v, '__getitem__') else v for k, v in test_batch.batch.items()}
                    task_batch = DataProto.from_dict(tensors=task_batch_dict)
                    for key, value in compute_length_metrics(task_batch).items():
                        length_metrics_lst[task][key].append(value)
            else:
                # Fallback to original behavior if no task information
                for key, value in reward_metrics.items():
                    reward_metrics_lst["overall"][key].extend(value)
                
                for key, value in compute_length_metrics(test_batch).items():
                    length_metrics_lst["overall"][key].append(value)

        self.actor_rollout_ref_wg.release_rollout_engine()
        self._maybe_log_val_generations(sample_inputs, sample_outputs, sample_labels, sample_scores)
        
        # Modified: Calculate task-specific and overall validation scores
        all_reward_tensors = torch.cat(reward_tensor_lst, dim=0)
        self.val_reward_score = all_reward_tensors.sum(-1).mean().item()
        
        # Prepare final metrics
        final_metrics = {"val/reward_score": self.val_reward_score}
        
        if "task" in test_batch.non_tensor_batch:
            # Add task-specific metrics
            overall_reward_metrics = defaultdict(list)
            overall_length_metrics = defaultdict(list)
            
            for task, task_reward_metrics in reward_metrics_lst.items():
                task_reduced_metrics = reduce_metrics(task_reward_metrics)
                for key, value in task_reduced_metrics.items():
                    final_metrics[f"val/{key}_reward_{task}"] = value
                    overall_reward_metrics[key].append(value)
            
            for task, task_length_metrics in length_metrics_lst.items():
                task_reduced_metrics = reduce_metrics(task_length_metrics)
                for key, value in task_reduced_metrics.items():
                    final_metrics[f"val_{key}_{task}"] = value
                    overall_length_metrics[key].append(value)
            
            # Add overall metrics (average across tasks)
            for key, values in overall_reward_metrics.items():
                final_metrics[f"val/{key}_reward_overall"] = sum(values) / len(values)
            
            for key, values in overall_length_metrics.items():
                final_metrics[f"val_{key}_overall"] = sum(values) / len(values)
        else:
            # Fallback to original behavior
            for task_metrics in reward_metrics_lst.values():
                task_reduced_metrics = reduce_metrics(task_metrics)
                for key, value in task_reduced_metrics.items():
                    final_metrics[f"val/{key}_reward"] = value
            
            for task_metrics in length_metrics_lst.values():
                task_reduced_metrics = reduce_metrics(task_metrics)
                for key, value in task_reduced_metrics.items():
